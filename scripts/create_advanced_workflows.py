"""Generate advanced node recipes and the partial-person diagnostic fixture."""
...
API = {
    ...
    "3": {"class_type": "S3F_PreviewExport", "inputs": {"project": ["2", 0], "filename": "motion"}},
    "4": {"class_type": "LoadVideo", "inputs": {"file": ""}},
}
...
def write_workflow(name, workflow, api):
    ...
    if name == 'partial_person_reference':
        directory = api_directory = ROOT / 'tests/fixtures/workflows'
    else:
        directory = ROOT / 'extras/advanced'
        api_directory = directory / 'api'
    for folder in (directory, api_directory):
        folder.mkdir(parents=True, exist_ok=True)
    (directory / f'{name}.json').write_text(json.dumps(workflow, indent=2) + '\n')
    (api_directory / f'{name}.api.json').write_text(json.dumps(api, indent=2) + '\n')
...
def core_workflow():
    ...
    api = {
        ...
        "8": {"class_type": "S3F_PreviewExport", "inputs": {"project": ["7", 0], "filename": "motion_core"}},
    }
    ...
    nodes = [
        ...
        make_node(8, api["8"], [2470, 150], [1100, 980], [port("project", "S3F_MOTION_PROJECT", 8)],
                  [output("project_path", "STRING", None)], ["motion_core"], "7 · Review, edit & export"),
    ]
...
def partial_person_workflow():
    ...
    workflow = {'last_node_id': 7, 'last_link_id': 7, 'nodes': nodes, 'links': links, 'groups': groups,
                'config': {}, 'extra': {'ds': {'scale': .45, 'offset': [30, 20]}}, 'version': .4}
    write_workflow('partial_person_reference', workflow, api)
...
def masked_workflow():
    ...
    api = {
        "1": {"class_type": "LoadVideo", "inputs": {"file": ""}},
        "2": {"class_type": "LoadVideo", "inputs": {"file": ""}},
