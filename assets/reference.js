...
import {correctedMotion,curveBuckets} from "./reference-edit.mjs";
...
let tracking=false,trackPending=null;
...
function times(){return project?.data?.times_ms||[0]}
function sourceTime(i){return project.data?Number(project.data.source_pts[i].split("/").reduce((a,b)=>a/Number(b))):project.first_source_ms/1000}
...
function seek(i){pause();index=Math.max(0,Math.min(times().length-1,i));video.currentTime=sourceTime(index);reveal();draw()}
...
function rebuild(){
    ...
    timelineLOD=null;
    if(project.data){
        try{motion=correctedMotion(times(),project.data.auto_shift_xy,project.data.reasons.map(r=>r==="consensus"),project.data.anchor_xy,config.sections)}catch(error){fail(error)}
    }
    extent=[0,0];for(const p of motion?.shifts||[])for(let j=0;j<2;j++)extent[j]=Math.max(extent[j],Math.abs(p[j]));
    if(motion){motion.counts={tracked:0,manual:0,held:0};motion.quality.forEach(q=>motion.counts[q]++)}
    ...
    $("seek").max=times().length-1;$("pointCount").textContent=`${config.points.length} starting points`;
    $("track").disabled=tracking;
    $("play").disabled=!project.data;$("previous").disabled=!project.data;$("next").disabled=!project.data;
...
function draw(){
    ...
    const t=times()[index]||0;$("seek").value=index;$("time").textContent=`${(t/1000).toFixed(3)} s · source ${sourceTime(index).toFixed(3)} s · frame ${index+1}/${times().length}`;
    ...
    const stale=project.data&&(!same(config.points,project.config.points)||!same(config.crop_xywh,project.config.crop_xywh));
    const points=project.data&&!stale&&$("mode").value!=="points"?project.data.points[index]:config.points;
...
function drawTimeline(){
    const [ctx,w,h]=prepare($("timeline"));if(!motion)return;
    ...
    if(!timelineLOD||timelineLOD.width!==w||timelineLOD.a!==a||timelineLOD.b!==b){
...
$("sourceCanvas").onpointerdown=e=>{if(e.button!==0||video.seeking)return;const p=position(e);if(!p)return;
    if($("mode").value==="crop"){drag={start:p,end:p};$("sourceCanvas").setPointerCapture(e.pointerId)}
    else if($("mode").value==="points"){snapshot();config.points.push(p);rebuild()}
...
$("sourceCanvas").oncontextmenu=e=>{e.preventDefault();if($("mode").value!=="points")return;const p=position(e);if(!p||!config.points.length)return;let best=-1,distance=12/sourceMap.scale;config.points.forEach((v,i)=>{const d=Math.hypot(v[0]-p[0],v[1]-p[1]);if(d<distance){best=i;distance=d}});if(best>=0){snapshot();config.points.splice(best,1);rebuild()}};
$("mode").onchange=()=>{if(["points","crop"].includes($("mode").value))seek(0);if($("mode").value==="correct"&&!project.data){$("mode").value="seek";status("Select starting points and queue tracking before correcting sections")}draw()};
$("zoom").onchange=draw;$("clearPoints").onclick=()=>{snapshot();config.points=[];seek(0);rebuild()};
...
$("timeline").onclick=e=>{if(!motion)return;const r=$("timeline").getBoundingClientRect(),f=Math.max(0,Math.min(1,(e.clientX-r.left-18)/(r.width-36))),[a,b]=view();seek(nearest(times(),a+f*(b-a)))};
...
function tick(){if(playing&&project.data){const offset=sourceTime(0);index=nearest(times(),(video.currentTime-offset)*1000);if(video.currentTime>=sourceTime(times().length-1)){pause();index=times().length-1}reveal();draw()}requestAnimationFrame(tick)}requestAnimationFrame(tick);
function validate(){
    if(config.points.length<3)throw new Error("Select at least three starting points");
    const [x,y,w,h]=config.crop_xywh;if(config.points.some(p=>p[0]<x||p[1]<y||p[0]>x+w-1||p[1]>y+h-1))throw new Error("Some starting points are outside the crop. Move the crop or remove those points.");
    if(project.data)correctedMotion(times(),project.data.auto_shift_xy,project.data.reasons.map(r=>r==="consensus"),project.data.anchor_xy,config.sections);
}
...
window.s3fReferenceApply=()=>{
    ...
    try{
        ...
        const timeout=setTimeout(()=>finishApply(new Error("ComfyUI did not acknowledge the settings. Keep this tab open and apply again.")),5000);
        ...
        try{workflowHost().postMessage({type:"s3f-reference-apply",node,reference:project.id,request,config:sent},location.origin)}catch(error){finishApply(error)}
        return promise;
