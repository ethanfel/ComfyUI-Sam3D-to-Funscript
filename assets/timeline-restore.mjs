import {validateOrientation} from './reference-edit.mjs?v=orientation-1';
...
export function restoreCandidate(raw,info,clock) {
    ...
    for(const lane of ['tracking','stabilization'])data[lane]=data[lane].map(r=>{
        ...
        if(lane==='tracking'){
            ...
            if(ref.transform_mode!==undefined&&!['translation','similarity','orientation'].includes(ref.transform_mode))throw new Error('Invalid stabilization correction mode.');
            if(ref.orientation||ref.transform_mode==='orientation')validateOrientation(ref.orientation,count,[info.width,info.height],false);
