...
**New motion projects default to Adaptive Auto for L0 stroke.** Each anchor project fits its own direction, origin and range over time. Mouth, left hand, right hand and detailed anchor overrides use their own coordinates; they do not inherit another track's calibration. The other five channels keep their distinct component mappings. Applying Auto to several translation channels in the same anchor project selects the same translation; rotation channels likewise share that anchor's rotation signal.

**Auto fit mode → Adaptive · per anchor** fits overlapping three-second windows, with calibration knots every half-second. It blends local direction, origin and gain instead of using one direction and range for the entire clip. Each window aims to place its filtered extrema at 5–95, subject to minimum ranges of **0.04 m for translation / 10° for rotation**. Exact stillness holds its position; calibration adjustment is bounded by measured movement. Cuts, invalid poses and timestamp gaps separate the fits. The read-only **Local full-scale range** displays the range at the playhead, while the saved scalar range is its median. Center stays at 50. This preserves smaller later movements when the anchor changes activity or direction, without marking an interval or rerunning SAM3D.

**Auto fit mode → Whole clip** retains one direction per uninterrupted span and one range/center for the clip. It fits the central 90% approximately into 10–90, with the same minimum ranges, and may expand the range to keep Center within 0–100. This mode enables manual range and center controls. Choose a source component and click **Regenerate selected axis** to apply those values. **Auto fit selected axis** refits using the selected mode. Both operations preserve Invert and every other channel, replace edits on the selected curve, and support Undo. Locked tracks remain protected across reruns.

To keep an explicit direction, choose **Source component → Up, Forward, or Left / right**, then click **Fit selected component**. This fits that component’s range and center without switching back to Auto. For example, Left / right can expose sideways movement when Auto follows a larger depth ramp. This does not remove camera rotation or pose errors. The fit uses valid cached samples from the selected source track, keeps Invert, supports Undo, and leaves Main unchanged until you apply the source again. Auto fit mode only applies when Source component is Auto.

Existing saved projects retain their curves when opened. Clicking **Auto fit selected axis** upgrades a project from before adaptive calibration to the new mode. New node runs use Adaptive by default, reusing the pose cache. The node, browser regeneration and standalone exports use matching calculations; no video decoding or model inference is needed for calibration. The blue arrow in the 3D pose view shows the current direction and follows Invert. On a rotation channel it shows the rotation axis and uses degrees.

For a separately authored section, select an interval on a source row and click **Fit selection as track**. It creates a new row with a local neutral position and a whole-window fit into 5–95, keeping the minimum range and Invert setting. The original source row and main remain intact. Review it, then use **Use selection in main** with a blend or manual join. Regeneration stays within the row's interval; reassigning its project or source axis resets it to the new project's full curve. Window calibration and pose provenance survive Undo, saving, offline reopening and removal of the source row after insertion into main.

Automatic direction uses principal-component fitting of short-window displacements, sampled uniformly at up to 30 Hz. Removing mean displacement reduces constant-drift influence; limiting displacement-vector lengths reduces outlier influence. Short or constant-speed spans fall back to position variation. The target's neutral torso orientation resolves initial polarity, and consecutive adaptive windows align their signs. Mixed motion uses a torso-axis fallback. **Directional share describes measured variation, not tracking confidence.** Camera motion, occlusion, depth errors, hand articulation and movement slower than the fitting window can still mislead the result. Review transitions and clipping; larger curves do not prove more accurate poses or a correct contact axis.

On the cached 16.7-second hand test clip, the left-hand script after 9.5 seconds increased from **7 to 90 position units** with Adaptive Auto, with zero filtered clipping across the clip. This tests motion calibration, not agreement with ground truth. Regression checks cover separate anchors, stillness, small jitter, changing directions, gaps, inversion, legacy projects and Python/browser parity. A synthetic one-hour sequence at 30 fps took about 5 seconds to fit in Python and 1 second in JavaScript on the development machine, excluding pose extraction, serialization and drawing.

The equivalent default L0 configuration is:

```json
{"axis_settings": {"L0": {"component": "auto", "auto_fit": true, "calibration": "adaptive"}}}
```

Set `calibration` to `clip` for a whole-clip fit. Set `auto_fit` to `false` to keep a specified `range` and `center`. Providing an explicit `component`, `range`, or `center` disables automatic fitting unless `auto_fit: true` is explicitly supplied. An empty configuration uses the new L0 defaults.
...
2. Every connected project adds a source track. The default **Section blocks · one row** view places each curve inside its analysed time interval, leaving unprocessed gaps empty. Click a block heading to select its range, or click its curve to seek and edit. **Overlapping anchors** switches between existing tracks covering that interval without changing their curves or calibration. The **Section** selector and previous/next buttons reach short or offscreen sections. Choose **Separate rows** to compare several curves at once. The selected track keeps its **Project**, **Axis**, name, lock, copy and remove controls. **Project** or **Axis** reassignment resets that track to the chosen project's calibration and curve; Undo restores its previous edits. **Add track** creates another independently editable track, including alternate calibrations of the same project.
...
5. Click **Copy selection · all axes** on the source row, or use **Use selection in main** above the timeline. The source row remains selected for copying when you inspect main. Blue shading and the **Copy source** badge identify it; dashed lines on other rows only align time. Copying updates **all matching enabled main axes** from that row’s anchor project: L0 → L0, L1 → L1, L2 → L2, R0 → R0, R1 → R1 and R2 → R2. The row’s displayed axis uses its edited curve and calibration; the other axes come from its assigned source project. The Main axis dropdown only chooses the displayed output, so it does not remap or restrict copying. Locked main axes and axes missing from the source stay unchanged; the controls list the destination axes and any protected axes. An out-of-bounds selection or invalid curve cannot partially update main. **Blend** crossfades from the existing main into the source and back inside the selected interval; the duration is per boundary, capped at half the selection. **Cut · manual join** inserts the source directly with one-millisecond boundary steps. Select main and drag/add/delete points to shape a join manually. Both methods preserve the original video timestamps and leave the rest of main in place; new boundary samples are rounded to integer positions. Blend sampling targets 0.25 position units of interpolation error before rounding, with a one-millisecond minimum step.
6. **Use whole track as main · all axes** replaces every matching unlocked main axis with the source’s full curve, applying edits on the row’s displayed axis. **Undo** restores all axes from a section insert or whole-track replacement together, as well as calibration, point edits and track changes.
...
Connected projects must reference the **same original video**. Different sampling rates, analysed trims and per-person masks can share that timeline. Section copying runs from the selected source's first frame through its recorded duration, including the final frame's held value. You can drag to the right edge or use **Select track range** to include it. Selection-fitted tracks keep their chosen interval even when its boundaries fall between frames; local fitting still requires at least two analysed frames. Selection and export use the same integer-millisecond rounding. Whole-track replacement holds its endpoint values outside the authored action range. There is no clip retiming or automatic detection of when to switch anchors. Shared pose arrays are stored once in the project, while each source keeps its own motion/calibration. Additional curves still consume memory; offscreen source rows skip drawing during playback.
...
Each source row and each main axis has a **Lock / Unlock** button. A locked track keeps its curve, calibration, source assignment and pose provenance through reruns, changed upstream motion, added inputs, browser reloads and project exports. It cannot be dragged, regenerated, inverted, reassigned or removed. A locked main axis is skipped during section insertion and whole-track replacement; other unlocked axes still update. If every matching axis is locked, copying is disabled. You can still seek, select time, inspect a locked source, derive a new fitted row from it, or copy it into an unlocked main. **Unlock** is explicit; changing a lock clears Undo history so Undo cannot reach behind it and replace protected work.

**Unlocked full-length source tracks and unassembled main axes follow changed upstream inputs**, even after editing or selecting a project manually. Lock a finished curve to keep its exact version. An identical input or appending another anchor preserves existing edits; local section tracks and assembled main curves remain snapshots. Generated row names follow the processing region and anchor of their assigned source, including renamed cropped regions. Custom names remain unchanged.

Opening the **Motion Studio** workspace tab checks for the latest saved processing result, including updates missed while the tab was hidden. This keeps the existing editor document and handles pending edits through the normal save and conflict recovery process.

Recreating a processing region at the same interval creates a new detection even if you reuse its name. When that result arrives, the section view follows an unambiguous replacement of the removed region. The row and selectors distinguish **Current detection** from **Saved detection**, so older results remain accessible. Choosing a saved result explicitly keeps it selected on subsequent refreshes. Main copies and edited curves retain their contents; use the copy controls to apply the replacement to an authored Main section.

For a main assembled by the **Processing Timeline**, editing or copying one section no longer stops automatic insertion elsewhere. Completed sections fill unused flat gaps, while existing sections, patterns, custom gap motion and locked main axes stay intact. New inserts blend at both edges inside their own range. Reruns can also insert a completed source row that an older version omitted from main.

The Project dropdown separates **Latest inputs** from **Saved versions used in this timeline**. Saved versions have distinct labels and remain only while referenced by a track or main section. They are retained project data, not pending executions. The latest-input mapping survives restarts and reverting an upstream input to an older result. Cache-hit flags and runtime timings do not create motion revisions.
...
Selection, layout and collapse changes recover automatically when another view or run updates the saved session. Competing authored edits pause saving and stay in their respective views. **Retry save** retries a failed connection or checks the latest revision again. **Download draft & load saved** downloads the complete current draft as project JSON, then loads the latest saved project without overwriting it. Use **Open project** to inspect the downloaded draft later. The same recovery handles a stale tab left open across a rerun; workflow execution waits while unresolved authored edits remain.
