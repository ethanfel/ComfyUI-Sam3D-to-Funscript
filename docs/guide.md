...
Recreating a processing region at the same interval creates a new detection even if you reuse its name. When that result arrives, the section view follows an unambiguous replacement of the removed region. The row displays **Latest detection** or **Previous detection**. **Section** chooses a zone once, **Anchor** lists its latest candidates, and **Previous detections** holds the older choices. **Show latest detection** returns from a previous result to its matching current anchor. Choosing a saved result explicitly keeps it selected on subsequent refreshes. Main copies and edited curves retain their contents; use the copy controls to apply the replacement to an authored Main section.
