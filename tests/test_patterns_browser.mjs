...
for(let i=0;i<audioRate*12;i++)mixPCM.writeInt16LE(Math.round(22000*Math.sin(i/audioRate*2*Math.PI*440)*(.25+.6*i/(audioRate*12))*(.3+.7*(1+Math.cos(i/audioRate*4*Math.PI))/2)),44+i*2);
...
try{
    ...
    // Sound browsing requires neither a drum file nor a generated block.
    await a.set('#beatSound','kick-deep');const beforeQuickSound=await saved();
    assert.equal(beforeQuickSound.audio_patterns,undefined);
    assert.equal(await disabled('beatPreviewSound'),false);assert.equal(await disabled('beatListen'),true);
    await a.click('#beatPreviewSound');
    await until(()=>a.evaluate("!document.querySelector('#beatAudition').paused && document.querySelector('#beatAudition').duration===2.5"),'standalone sound sample');
    assert.equal(await a.evaluate("document.querySelector('#beatPreviewSound').textContent"),'Stop preview');
    assert.match(await a.evaluate("document.querySelector('#beatStatus').textContent"),/Deep kick/);
    const afterQuickSound=await saved();assert.deepEqual(afterQuickSound.scripts,beforeQuickSound.scripts);
    assert.deepEqual(afterQuickSound.timeline,beforeQuickSound.timeline);assert.equal(afterQuickSound.audio_patterns,undefined);
    await until(()=>a.evaluate("document.querySelector('#beatAudition').hidden"),'sound sample finishes');
    assert.equal(await a.evaluate("document.querySelector('#beatPreviewSound').getAttribute('aria-pressed')"),'false');
    await a.click('#beatPreviewSound');
    await until(()=>a.evaluate("!document.querySelector('#beatAudition').paused"),'replay sample');
    await a.click('#beatPreviewSound');assert.equal(await a.evaluate("document.querySelector('#beatAudition').hidden"),true);
    await a.evaluate("document.querySelector('#beatPreviewSound').click();document.querySelector('#beatPreviewSound').click();document.querySelector('#beatPreviewSound').click()");
    await until(()=>a.evaluate("!document.querySelector('#beatAudition').paused && !document.querySelector('#beatAudition').hidden"),'rapid preview restart');
    await a.set('#beatSound','auto');assert.equal(await a.evaluate("document.querySelector('#beatAudition').hidden"),true);
    await a.click('#beatPreviewSound');
    await until(()=>a.evaluate("document.querySelector('#beatStatus').textContent.includes('default kick')"),'auto sound without drum data');
    await a.click('#beatPreviewSound');
    report.checks.push('Quick percussion preview needs no audio or motion block, changes no edits, ends/stops cleanly and tolerates rapid clicks');
    const beforeAudio=await saved();await loadAudio(audioPath);
    await a.click('#beatPreviewSound');
    await until(()=>a.evaluate("!document.querySelector('#beatAudition').paused"),'auto sample from analysis');
    assert.match(await a.evaluate("document.querySelector('#beatStatus').textContent"),/nearest drum hit/);
    await a.click('#beatPreviewSound');
    ...
    assert.equal(audioProject.audio_patterns.analysis.version,4);
    ...
    assert.deepEqual(await a.evaluate("['beatRhythm','beatDensity','beatLowFocus','beatMaxHits','beatAmplitude'].map(id=>document.getElementById(id).value)"),['simplify','1','65','3','80']);
    assert.equal(await disabled('beatCycle'),true);
    assert.equal(await a.evaluate("document.querySelectorAll('#beatSound option').length"),15);
    assert.equal(await a.evaluate("document.querySelector('#beatSound').value"),'auto');
    assert.match(await a.evaluate("document.querySelector('#beatSoundHint').textContent"),/kick/i);
    const beforeSound=await saved();
    await a.set('#beatSound','tom-low');
    assert.equal(await disabled('beatSave'),false,'Changing percussion preserves the motion draft');
    await a.click('#beatPreviewSound');
    await until(()=>a.evaluate("!document.querySelector('#beatAudition').paused"),'sound preview with a motion draft');
    assert.equal(await disabled('beatSave'),false,'Quick sound preview preserves the generated motion draft');
    await a.click('#beatPreviewSound');
    const afterSound=await saved();assert.equal(afterSound.preview.beat_sound,'tom-low');
    assert.deepEqual(afterSound.audio_patterns,beforeSound.audio_patterns);assert.deepEqual(afterSound.scripts,beforeSound.scripts);
    await a.click('#beatListen');
    await until(()=>a.evaluate("!document.querySelector('#beatAudition').paused"),'tom audition');
    await a.set('#beatSound','auto');
    assert.equal(await a.evaluate("document.querySelector('#beatAudition').hidden"),true,'Sound changes stop the old audition');
    assert.equal(await disabled('beatSave'),false);
    await a.click('#beatListen');
    await until(()=>a.evaluate("!document.querySelector('#beatAudition').paused && !document.querySelector('#beatAudition').hidden"),'rhythm audition');
    assert.equal(await a.evaluate("document.querySelector('#video').paused"),true);
    await a.set('#beatDensity','0.5');
    assert.equal(await a.evaluate("document.querySelector('#beatAudition').hidden"),true,'Changing rhythm stops an obsolete audition');
    assert.equal(await disabled('beatSave'),true,'Rhythm changes invalidate the old motion preview');
    await a.set('#beatDensity','1');await a.click('#beatPreview');
    const rhythmFolder=fs.mkdtempSync(path.join(temporary,'rhythm-'));
    await a.call('Browser.setDownloadBehavior',{behavior:'allow',downloadPath:rhythmFolder});
    await a.click('#beatDownloadEvents');await a.click('#beatDownloadWav');
    await until(()=>fs.readdirSync(rhythmFolder).filter(f=>/\.(json|wav)$/.test(f)).length===2,'rhythm exports');
    const rhythmEvents=JSON.parse(fs.readFileSync(path.join(rhythmFolder,fs.readdirSync(rhythmFolder).find(f=>f.endsWith('.json')))));
    assert.equal(rhythmEvents.start_ms,3000);assert.equal(rhythmEvents.end_ms,7000);
    assert.ok(rhythmEvents.events.every(p=>p.video_at===p.at+3000));
    assert.equal(rhythmEvents.version,2);assert.equal(rhythmEvents.sound,'auto');
    assert.ok(rhythmEvents.events.every(p=>p.timbre&&p.percussion.match==='stem-timbre'));
    assert.ok(rhythmEvents.events.every(p=>p.percussion.voices[0].sound.includes('kick')));
    report.checks.push('Percussion kit and automatic timbre recipes preserve motion drafts and export exact event times');
    const rhythmWav=fs.readFileSync(path.join(rhythmFolder,fs.readdirSync(rhythmFolder).find(f=>f.endsWith('.wav'))));
    assert.equal(rhythmWav.toString('ascii',0,4),'RIFF');assert.equal(rhythmWav.length,44+22050*4*2);
    ...
    assert.equal(audioProject.audio_patterns.sections[0].settings.rhythm,'simplify');
    assert.deepEqual(audioProject.audio_patterns.sections[0].events.map(p=>p.at),rhythmEvents.events.map(p=>p.video_at));
    report.checks.push('Rhythm controls, invalidation, actual audio audition and exact persisted-event WAV/JSON exports');
    ...
    await a.click('#beatCatalogue summary');
    assert.equal(await a.evaluate("document.querySelectorAll('.beat-pattern-card').length"),28);
    await a.set('#beatFamily','Pulses');
    assert.equal(await a.evaluate("[...document.querySelectorAll('.beat-pattern-card')].every(b=>b.querySelector('svg polyline'))"),true);
    await a.click('.beat-pattern-card[data-shape="Sharp Rebound"]');
    assert.deepEqual(await a.evaluate("['beatMode','beatShape'].map(id=>document.getElementById(id).value)"),['manual','Sharp Rebound']);
    await a.click('#beatPreview');assert.equal(await disabled('beatSave'),false);
    await a.set('#beatFamily','Flowing');assert.equal(await disabled('beatSave'),true,'Changing the matching family invalidates the old preview');
    await a.click('#beatAutoMatch');assert.equal(await a.evaluate("document.querySelector('#beatMode').value"),'waveform');
    assert.match(await a.evaluate("document.querySelector('#beatModeHint').textContent"),/volume envelope/);
    assert.equal(await a.evaluate("document.querySelector('#beatShapeControl').hidden"),true,'Auto mode cannot display a stale manual shape');
    assert.ok(await a.evaluate("document.querySelectorAll('.beat-pattern-card.matched').length")>0,'The catalogue identifies the automatic matches');
    assert.match(await a.evaluate("document.querySelector('#beatDecisions').textContent"),/template similarity/);
    assert.match(await a.evaluate("document.querySelector('#beatDecisions').textContent"),/Next matches/);
    await a.click('#beatSave');audioProject=await saved();
    assert.equal(audioProject.audio_patterns.sections[0].settings.mode,'waveform');
    assert.equal(audioProject.audio_patterns.sections[0].settings.patternFamily,'Flowing');
    assert.ok(audioProject.audio_patterns.sections[0].decisions.every(d=>d.source==='dual'&&d.alternatives.length===3));
    assert.deepEqual(audioProject.scripts,beforeAudio.scripts,'Auto matching remains an audio source until copied to Main');
    await a.set('#beatSection','beat_0');
    assert.deepEqual(await a.evaluate("['beatMode','beatFamily'].map(id=>document.getElementById(id).value)"),['waveform','Flowing']);
    await a.evaluate("document.querySelector('#beatCatalogue').scrollIntoView({block:'center'})");await pause(80);
    fs.writeFileSync(path.join(output,'audio-pattern-catalogue.png'),Buffer.from((await a.call('Page.captureScreenshot')).data,'base64'));
    await a.click('#beatCatalogue summary');
    report.checks.push('28 visual motion patterns, family filtering, manual selection, envelope matching with ranked alternatives, settings persistence and preview-only changes');
    ...
    await a.set('#beatSound','wood');await saved();
    const files=await download(a),snapshot=JSON.parse(files["project.json"]);
    assert.equal(snapshot.preview.beat_sound,'wood');
    ...
    assert.equal(await a.evaluate("document.querySelector('#beatSound').value"),'wood','Percussion preference survives an offline export');
    await a.set('#beatSound','auto');
    ...
    assert.equal(await a.evaluate("document.querySelector('#beatMode').value"),'waveform','Auto matching works in the offline viewer');
    assert.equal(await a.evaluate("document.querySelector('#beatFamily').value"),'Flowing');
    await a.click('#beatCatalogue summary');
    await a.call('Emulation.setDeviceMetricsOverride',{width:360,height:1000,deviceScaleFactor:1,mobile:false});await pause(80);
    assert.equal(await a.evaluate('document.documentElement.scrollWidth<=innerWidth'),true,'The visual catalogue fits a phone-width viewport');
    await a.click('#beatCatalogue summary');
    await a.click('#beatPreviewSound');await until(()=>a.evaluate("!document.querySelector('#beatAudition').paused"),'offline quick sound preview');
    assert.equal(await disabled('beatSave'),false);await a.click('#beatPreviewSound');
    await a.click('#beatListen');await until(()=>a.evaluate("!document.querySelector('#beatAudition').paused"),'offline rhythm audition');
    await a.click('#beatDiscard');assert.equal(await a.evaluate("document.querySelector('#beatAudition').hidden"),true);
    await a.click('#beatPreview');
    ...
    await a.set('#beatSection','beat_0');assert.equal(await a.evaluate("document.querySelector('#beatRhythm').value"),'original','Old blocks reopen with original timing');
    assert.equal(await a.evaluate("document.querySelector('#beatAmplitude').value"),'80','Old amplitude maps to full stroke length');
    ...
    await a.set('#beatSound','hat-open');
    const disjointFolder=fs.mkdtempSync(path.join(temporary,'percussion-gaps-'));
    await a.call('Browser.setDownloadBehavior',{behavior:'allow',downloadPath:disjointFolder});
    await a.click('#beatDownloadWav');await a.click('#beatDownloadEvents');
    await until(()=>fs.readdirSync(disjointFolder).filter(f=>/\.(wav|json)$/.test(f)).length===2,'disjoint percussion export');
    const gapWav=fs.readFileSync(path.join(disjointFolder,fs.readdirSync(disjointFolder).find(f=>f.endsWith('.wav'))));
    const gapEvents=JSON.parse(fs.readFileSync(path.join(disjointFolder,fs.readdirSync(disjointFolder).find(f=>f.endsWith('.json')))));
    assert.deepEqual(gapEvents.ranges_ms,[[0,3000],[7000,10000]]);
    assert.ok(gapWav.subarray(44+3000*22050/1000*2,44+7000*22050/1000*2).every(x=>x===0),'Open-hat tails leave unselected blocks silent');
    report.checks.push('Disjoint percussion WAV/JSON exports retain exact selected ranges and silence between blocks');
    ...
    // Folder review controls use the real Motion Studio and never author comparison curves.
    const mainBeforeComparison=structuredClone(state.project.scripts);
    await b.evaluate(`${studio}.s3fFolderIssues([{reason:'Inspect this range',start_ms:1000,end_ms:2500}]);${studio}.s3fFolderSelectRange(1000,2500,{play:false});${studio}.s3fFolderCompare({name:'Gentle version',scripts:{L0:{actions:[{at:0,pos:10},{at:12000,pos:90}]}}});${studio}.s3fFlush()`);
    assert.deepEqual(await b.evaluate(`['selectionStart','selectionEnd'].map(id=>Number(${studio}.document.getElementById(id).value))`),[1,2.5]);
    assert.match(await b.evaluate(`${studio}.document.querySelector('#folderComparison').textContent`),/Gentle version.*pink saved curve/);
    assert.deepEqual(state.project.scripts,mainBeforeComparison,'Version comparison does not replace Main or source scripts');
    await b.evaluate(`${studio}.s3fFolderPlaySelection()`);
    await until(()=>b.evaluate(`!${studio}.document.querySelector('#video').paused`),'review range playback');
    await until(()=>b.evaluate(`${studio}.document.querySelector('#video').paused`),'review range playback stops');
    assert.ok(await b.evaluate(`Math.abs(${studio}.document.querySelector('#video').currentTime-2.5)<.2`));
    await b.evaluate(`${studio}.s3fFolderCompare(null);${studio}.s3fFolderIssues([])`);
    assert.equal(await b.evaluate(`${studio}.document.querySelector('#folderComparison').hidden`),true);
    report.checks.push('Folder review flags select exact ranges; playback stops at their end; saved-version overlays leave Main scripts unchanged and can be hidden');
