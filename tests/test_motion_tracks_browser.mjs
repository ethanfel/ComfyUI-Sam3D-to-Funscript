...
try{
    ...
    const controlsBelowImage=()=>evaluate('(()=>{const v=document.querySelector("#video").getBoundingClientRect(),c=document.querySelector("#videoControls").getBoundingClientRect();return c.top>=v.bottom&&c.height>0&&c.right<=document.querySelector("#videoPanel").getBoundingClientRect().right;})()');
    assert.equal(await evaluate('originalVideo.controls'),false,'Native playback overlay cannot cover the image');
    assert.ok(await controlsBelowImage(),'Playback strip sits below the complete video image');
    await click('#videoPlay');await until(()=>evaluate('!originalVideo.paused'),'play button');
    await click('#videoPlay');await until(()=>evaluate('originalVideo.paused'),'pause button');
    await evaluate('document.querySelector("#videoSeek").value=1500;document.querySelector("#videoSeek").dispatchEvent(new Event("input"))');
    await until(()=>evaluate('!originalVideo.seeking&&Math.abs(originalVideo.currentTime-1.5)<.01'),'seek strip');
    assert.equal(await evaluate('Number(document.querySelector("#time").dataset.ms)'),1500,'Transport seeking updates the shared motion clock');
    await evaluate('originalVideo.dispatchEvent(new KeyboardEvent("keydown",{key:" ",code:"Space",bubbles:true}))');
    await until(()=>evaluate('!originalVideo.paused'),'Space on video');await click('#video');
    assert.equal(await evaluate('originalVideo.paused'),true,'Clicking video pauses without showing a control overlay');
    await click('#videoMute');await until(()=>evaluate('originalVideo.muted&&document.querySelector("#videoMute").textContent==="Unmute"'),'mute');
    await evaluate('document.querySelector("#videoVolume").value=.35;document.querySelector("#videoVolume").dispatchEvent(new Event("input"))');
    assert.ok(await evaluate('!originalVideo.muted&&Math.abs(originalVideo.volume-.35)<.001'),'Volume slider controls the same media element');
    await click('#videoFullscreen');await until(()=>evaluate('document.fullscreenElement?.id==="videoPanel"'),'video fullscreen');
    assert.ok(await controlsBelowImage(),'Fullscreen keeps controls below the image');
    fs.writeFileSync(output+'/video-fullscreen.png',Buffer.from((await call('Page.captureScreenshot')).data,'base64'));
    await click('#videoFullscreen');await until(()=>evaluate('!document.fullscreenElement'),'exit video fullscreen');
    const pixels=()=>evaluate(`(()=>{const c=document.querySelector('#robot'),d=c.getContext('2d').getImageData(0,0,c.width,c.height).data;let left=c.width,right=0,top=c.height,bottom=0;for(let y=0;y<c.height;y++)for(let x=0;x<c.width;x++)if(d[(y*c.width+x)*4+3]>64){left=Math.min(left,x);right=Math.max(right,x);top=Math.min(top,y);bottom=Math.max(bottom,y);}return {width:right-left,height:bottom-top,canvasWidth:c.width,canvasHeight:c.height};})()`);
    const fitted=await pixels();assert.ok(Math.max(fitted.width/fitted.canvasWidth,fitted.height/fitted.canvasHeight)>.6,'Default device framing fills its canvas');
    for(let i=0;i<4;i++)await click('#deviceZoomOut');const distant=await pixels();
    await click('#deviceFit');const refitted=await pixels();assert.ok(refitted.height>distant.height*1.7,'Fit recovers from an excessively distant view');
    await evaluate('originalVideo.currentTime=3;originalVideo.play()');await until(()=>evaluate('!originalVideo.paused&&!originalVideo.seeking'),'layout playback');
    await select('#deviceLayout','beside');await pause(100);
    assert.equal(await evaluate('document.querySelector("#devicePanel").parentElement.id'),'videoDeviceStage');
    assert.ok(await evaluate(`(()=>{const v=document.querySelector('.video-wrap').getBoundingClientRect(),d=document.querySelector('#devicePanel').getBoundingClientRect();return d.left>=v.right&&d.left-v.right<=10;})()`),'Compact device is immediately beside the video');
    fs.writeFileSync(output+'/device-beside.png',Buffer.from((await call('Page.captureScreenshot')).data,'base64'));
    await select('#deviceLayout','overlay');await select('#deviceCorner','bottom-left');await pause(100);
    assert.ok(await evaluate(`(()=>{const v=document.querySelector('#videoDeviceStage').getBoundingClientRect(),d=document.querySelector('#devicePanel').getBoundingClientRect();return d.left>=v.left&&d.left<v.left+12&&d.right<=v.right&&d.bottom<=v.bottom-7;})()`),'Overlay stays within the video area');
    assert.equal(await evaluate('document.querySelector("#deviceOptions").hidden'),true);
    await click('#deviceSettings');assert.equal(await evaluate('document.querySelector("#deviceOptions").hidden'),false);
    await select('#device','handy2');assert.equal(await evaluate('document.querySelectorAll("#readouts span").length'),1);
    await select('#device','sr6');await click('#deviceSettings');
    assert.equal(await evaluate('!originalVideo.paused&&document.querySelector("#video")===originalVideo&&mediaReloads===0'),true,'Changing device layout and controls keeps the same video playing');
    assert.equal(await evaluate('(()=>{const p=document.querySelector("#devicePanel");return p.scrollHeight<=p.clientHeight+1;})()'),true,'Compact preview fits its canvas and readouts without scrolling');
    const deviceBounds=()=>evaluate('(()=>{const r=document.querySelector("#devicePanel").getBoundingClientRect();return {x:r.x,y:r.y,width:r.width,height:r.height}})()');
    const anchoredDevice=await deviceBounds();
    const deviceGrip=await evaluate('(()=>{const r=document.querySelector("#deviceGrip h2").getBoundingClientRect();return {x:r.left+15,y:r.top+8}})()');
    for(const [type,x,y,buttons]of [['mousePressed',deviceGrip.x,deviceGrip.y,1],['mouseMoved',deviceGrip.x+230,deviceGrip.y-40,1],['mouseReleased',deviceGrip.x+230,deviceGrip.y-40,0]])await call('Input.dispatchMouseEvent',{type,x,y,button:'left',buttons,clickCount:1});
    const draggedDevice=await deviceBounds();
    assert.ok(Math.abs(draggedDevice.x-anchoredDevice.x-230)<2&&Math.abs(draggedDevice.y-anchoredDevice.y+40)<2,'Heading drag freely moves the overlay beside the subject');
    assert.equal(await evaluate('document.querySelector("#deviceCorner").value'),'custom');
    await select('#deviceLayout','panels');await select('#deviceLayout','overlay');
    assert.deepEqual(await deviceBounds(),draggedDevice,'Switching layouts keeps the free overlay position');
    await evaluate('document.querySelector("#deviceGrip h2").dispatchEvent(new KeyboardEvent("keydown",{key:"ArrowRight",shiftKey:true,bubbles:true}))');
    assert.ok(Math.abs((await deviceBounds()).x-draggedDevice.x-40)<2,'Keyboard moves the device window');
    const beforeOrbit=await deviceBounds(),robotGrip=await evaluate('(()=>{const r=document.querySelector("#robot").getBoundingClientRect();return {x:r.left+r.width/2,y:r.top+r.height/2}})()');
    for(const [type,x,y,buttons]of [['mousePressed',robotGrip.x,robotGrip.y,1],['mouseMoved',robotGrip.x+30,robotGrip.y+20,1],['mouseReleased',robotGrip.x+30,robotGrip.y+20,0]])await call('Input.dispatchMouseEvent',{type,x,y,button:'left',buttons,clickCount:1});
    assert.deepEqual(await deviceBounds(),beforeOrbit,'Dragging the model orbits without moving its window');
    assert.ok(await evaluate('JSON.parse(localStorage.getItem("s3f-device-layout:1")).position.x>0'),'Custom position is remembered in the browser');
    assert.equal(await evaluate('!originalVideo.paused&&document.querySelector("#video")===originalVideo&&mediaReloads===0'),true,'Moving the device preserves video playback');
    fs.writeFileSync(output+'/device-overlay.png',Buffer.from((await call('Page.captureScreenshot')).data,'base64'));
    await select('#deviceLayout','panels');
    assert.deepEqual(await evaluate('[...document.querySelector(".stage").children].map(e=>e.querySelector("video,#robot,#skeleton")?.id)'),['video','robot','skeleton'],'Panels mode restores the original layout');
    await select('#deviceLayout','overlay');await select('#deviceCorner','bottom-right');
    assert.equal(await evaluate('JSON.parse(localStorage.getItem("s3f-device-layout:1")).position'),null,'Choosing a corner resets the custom position');
    await click('#floatVideo');
    assert.equal(await evaluate('document.querySelector("#videoPanel").contains(document.querySelector("#robot"))'),true,'The compact device floats with the video');
    ...
    const beforeDeviceMove=await floatingBounds();
    await evaluate('document.querySelector("#deviceGrip h2").dispatchEvent(new KeyboardEvent("keydown",{key:"ArrowLeft",shiftKey:true,bubbles:true}))');
    assert.deepEqual(await floatingBounds(),beforeDeviceMove,'Moving the device does not move the floating video');
    const freeDevicePosition=await evaluate('JSON.parse(localStorage.getItem("s3f-device-layout:1")).position');
    ...
    assert.ok(await evaluate('(()=>{const s=document.querySelector("#videoDeviceStage").getBoundingClientRect(),p=document.querySelector("#devicePanel").getBoundingClientRect();return p.left>=s.left&&p.top>=s.top&&p.right<=s.right&&p.bottom<=s.bottom-7;})()'),'Free overlay stays inside the resized video');
    assert.ok(await controlsBelowImage(),'Narrow floating video keeps playback controls below the image');
    assert.deepEqual(await evaluate('JSON.parse(localStorage.getItem("s3f-device-layout:1")).position'),freeDevicePosition,'Resizing preserves the preferred position');
    await call('Emulation.setDeviceMetricsOverride',{width:1500,height:1000,deviceScaleFactor:1,mobile:false});
    ...
    assert.equal(await evaluate('(()=>{const p=document.querySelector("#devicePanel");return p.scrollHeight<=p.clientHeight+1;})()'),true,'Floating overlay remains usable after narrowing the window');
    ...
    checks.push('Fitted device camera, visible zoom/reset controls, beside/overlay/panels layouts, corner selection, compact device settings and uninterrupted playback');
    checks.push('Device header drag and keyboard positioning, separate model orbit, remembered placement, corner reset and clamping inside resized/floating video');
    checks.push('Playback strip below image in docked, narrow floating and fullscreen views; play/pause, video click/Space, seek clock sync and mute/volume work without native overlays');
    ...
    assert.equal(compactProject.preview.device_layout,'overlay');assert.equal(compactProject.preview.device_corner,'bottom-right');
    assert.deepEqual(compactProject.preview.device_position,freeDevicePosition);
    ...
    assert.equal(await evaluate('document.querySelector("#deviceLayout").value'),'overlay','Offline export restores device layout');
    assert.equal(await evaluate('document.querySelector("#deviceCorner").value'),'custom','Offline export restores free device placement');
    assert.equal(await evaluate('document.querySelector("#sourceLayout").value'),'sections');
    ...
    // Merge several old shots into one new detection, then recover its deleted row.
    const beforeRecovery=structuredClone(draft.project);
    const expandedSource=draft.project.timeline.sources.find(s=>s.id===replacementSource.id);
    expandedSource.data.metadata.processing_region.start_ms=0;
    expandedSource.geometry='replacement-full';
    const oldGeometry=replacementSource.geometry==='base'?draft.project:draft.project.timeline.geometries[replacementSource.geometry];
    draft.project.timeline.geometries['replacement-full']=structuredClone(oldGeometry);
    draft.project.timeline.geometries['replacement-full'].times_ms[0]=0;
    draft.revision++;await evaluate('window.s3fUpdate()');await select('#sectionTrack',replacementTrack.id);await flush();
    const visibleMerged=await evaluate('[...document.querySelectorAll("#sectionLabels button")].map(b=>b.dataset.track)');
    assert.equal(visibleMerged.includes(oldRow.id),false,'Shorter obsolete scenes do not cover the full-length replacement');
    await click('#tracks .section-current .remove-track');await flush();
    assert.equal(draft.project.timeline.tracks.some(t=>t.source===replacementSource.id),false);
    assert.equal(await evaluate('document.querySelector("#restoreLatest").hidden'),false);
    await evaluate('document.querySelector("#restoreLatest").scrollIntoView({block:"center"})');
    fs.writeFileSync(output+'/restore-latest.png',Buffer.from((await call('Page.captureScreenshot')).data,'base64'));
    const mainBeforeRestore=structuredClone(draft.project.scripts),rowsBeforeRestore=structuredClone(draft.project.timeline.tracks);
    await click('#restoreLatest');await flush();
    const recoveredRow=draft.project.timeline.tracks.find(t=>t.source===replacementSource.id);
    assert.ok(recoveredRow);assert.equal(await evaluate('document.querySelector("#sectionAnchor").value'),recoveredRow.id);
    assert.equal(await evaluate('document.querySelector("#restoreLatest").hidden'),true);
    assert.deepEqual(draft.project.scripts,mainBeforeRestore);assert.deepEqual(draft.project.timeline.tracks.slice(0,-1),rowsBeforeRestore);
    await click('#tracks .section-current .remove-track');await flush();await click('#addTrack');await flush();
    assert.equal(draft.project.timeline.tracks.at(-1).source,replacementSource.id,'Add track chooses missing latest input rather than first saved scene');
    draft.project.timeline.tracks=[];draft.project.timeline.active='main';draft.revision++;await evaluate('window.s3fUpdate()');
    await click('#addTrack');await flush();assert.ok(Object.values(draft.project.timeline.latest).includes(draft.project.timeline.tracks[0].source),'Add track picks a latest detection with all rows removed');
    await click('#restoreLatest');await flush();assert.ok(draft.project.timeline.tracks.some(t=>t.source===replacementSource.id),'Restore brings back the remaining latest detections');
    draft.project=beforeRecovery;draft.revision++;await evaluate('window.s3fUpdate()');
    checks.push('Full-length replacement hides obsolete scene boundaries; Restore latest and Add track recover deleted detections without changing Main or existing rows');
    ...
    // A replacement reaching a fractional video endpoint must keep the source
    // tail, rather than blending back to an obsolete high Main endpoint.
    draft.project=JSON.parse(fs.readFileSync(temp+'/project.json'));draft.revision++;
    const edgeTrack=draft.project.timeline.tracks[0],edgeSource=draft.project.timeline.sources.find(s=>s.id===edgeTrack.source);
    const exactEnd=edgeSource.data.metadata.duration_ms,edgeEnd=Math.round(exactEnd);
    draft.project.metadata.duration_ms=exactEnd;
    for(const script of Object.values(draft.project.scripts))script.actions=[{at:0,pos:85},{at:edgeEnd,pos:85}];
    edgeTrack.script.actions=[{at:36146,pos:30},{at:edgeEnd-500,pos:25},{at:edgeEnd-200,pos:12},{at:edgeEnd,pos:12}];
    draft.project.timeline.main.L0.regions=[{start:0,end:exactEnd,source:edgeTrack.source,axis:'L0',settings:structuredClone(edgeTrack.settings),join:'blend',blend_ms:200}];
    await call('Emulation.setDeviceMetricsOverride',{width:1500,height:1250,deviceScaleFactor:1,mobile:false});
    await call('Page.navigate',{url:pageURL});
    await until(()=>evaluate('document.querySelectorAll("#tracks .track").length===2'),'clip edge fixture');
    await select('#sectionTrack',edgeTrack.id);await click('#tracks .section-current .select-track-range');await flush();
    const edgeBefore=structuredClone(draft.project);
    await click('#tracks .section-current .copy-selection');await flush();
    const edgeAfter=structuredClone(draft.project);
    assert.equal(curveValue(edgeAfter.scripts.L0.actions,edgeEnd),12,'copy retains the source endpoint');
    assert.equal(curveValue(edgeAfter.scripts.L0.actions,edgeEnd-100),12,'no exit blend back to obsolete Main');
    assert.deepEqual(edgeAfter.timeline.main.L0.regions.map(r=>[r.start,r.end]),[[0,36146],[36146,edgeEnd]]);
    assert.equal(curveValue(edgeAfter.scripts.L0.actions,36000),85,'earlier Main stays unchanged');
    assert.deepEqual(edgeAfter.timeline.tracks,edgeBefore.timeline.tracks);
    await click('#undo');await flush();assert.deepEqual(draft.project.scripts,edgeBefore.scripts);
    assert.deepEqual(draft.project.timeline.main,edgeBefore.timeline.main);
    checks.push('Copying through fractional video end keeps source tail, preserves earlier Main and source tracks, and undoes completely');
    const deviceScripts=structuredClone(draft.project.scripts),deviceTracks=structuredClone(draft.project.timeline.tracks);
    await select('#device','handy2');await flush();
    assert.equal(draft.project.preview.device,'handy2','changing only the device saves its preview setting');
    assert.deepEqual(draft.project.scripts,deviceScripts);assert.deepEqual(draft.project.timeline.tracks,deviceTracks);
    draft.project.preview.device='sr6';draft.project.metadata.source.path='another-neutral-clip.mp4';draft.revision++;
    timeline.info.source=structuredClone(draft.project.metadata.source);
    await evaluate('window.s3fUpdate()');
    assert.equal(await evaluate('document.querySelector("#device").value'),'handy2','next clip cannot replace the user preference with its saved SR6');
    await call('Page.navigate',{url:pageURL});
    await until(()=>evaluate('document.querySelectorAll("#tracks .track").length===2'),'device preference reload');
    assert.equal(await evaluate('document.querySelector("#device").value'),'handy2');
    await evaluate(`(()=>{const frame=document.createElement('iframe');frame.id='device-prefetch-test';frame.dataset.prefetch='true';frame.src=${JSON.stringify(pageURL)};document.body.append(frame);})()`);
    await until(()=>evaluate('document.querySelector("#device-prefetch-test").contentDocument?.querySelectorAll("#tracks .track").length===2'),'second device preview');
    await select('#device','sr6');await flush();
    await until(()=>evaluate('document.querySelector("#device-prefetch-test").contentDocument.querySelector("#device").value==="sr6"'),'preloaded preview follows device change');
    await select('#device','handy2');await flush();
    await until(()=>evaluate('document.querySelector("#device-prefetch-test").contentDocument.querySelector("#device").value==="handy2"'),'preloaded preview follows latest device');
    await evaluate('document.querySelector("#device-prefetch-test").remove()');
    checks.push('Device choice saves without curve edits and survives next clip, reload, and already-loaded previews');
