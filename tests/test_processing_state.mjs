...
assert.ok(trackingResultCurrent({...r,isolate_subject:false},entry,[]),'default crop setting preserves old result freshness');
...
assert.equal(trackingResultCurrent({...r,candidate_people:[0]},entry,[]),false);
const auto={...r,candidate_people:[0],automatic:{version:1,suggest:true,people:[],review:[]}};
assert.ok(trackingResultCurrent(auto,{region:auto,stabilization_regions:[]},[]));
assert.equal(trackingResultCurrent({...auto,automatic:{...auto.automatic,suggest:false}},{region:auto,stabilization_regions:[]},[]),false);
assert.equal(trackingResultCurrent({...r,anchor:'mouth'},{region:r},[]),false,'known changes still invalidate legacy output');
for(const patch of [{isolate_subject:true},{anchor:'mouth'},{person:1},{smoothing_ms:80},{additional_anchors:['nose']},{rois:[[0,0,.5,1]]},{settings:{axis_settings:{L0:{invert:true}}}},{mask_anchor:{frame:1,strokes:[]}}])assert.equal(trackingResultCurrent({...r,...patch},entry,[]),false);
...
assert.equal(restoreCandidate({...plan,tracking:[{...r,isolate_subject:true}]},info,clock).tracking[0].isolate_subject,true);
assert.throws(()=>restoreCandidate({...plan,tracking:[{...r,isolate_subject:'false'}]},info,clock),/crop/);
