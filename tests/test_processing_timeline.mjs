...
assert.equal(changeRegion(meshPlan,meshId,{end_ms:30210},shortInfo,clock).tracking[0].mask_anchor,undefined);
...
const trimmed=changeRegion(meshPlan,meshId,{end_ms:30400},shortInfo,clock).tracking[0];
assert.equal(trimmed.mask_anchor.frame,3,'trimming the tail keeps a seed still inside the region');
const rebased=changeRegion(meshPlan,meshId,{start_ms:30090},shortInfo,clock).tracking[0];
assert.equal(rebased.mask_anchor.frame,2,'seed remains on the same source frame when In changes');
const preserved=changeRegion(markedPlan,'marked',{end_ms:30400},shortInfo,clock).stabilization[0];
assert.equal(preserved.reference.keyframes.length,3);
assert.deepEqual(preserved.reference.point_mask,marked.reference.point_mask);
const trimmedStart=changeRegion(markedPlan,'marked',{start_ms:30090},shortInfo,clock).stabilization[0];
assert.deepEqual(trimmedStart.reference.keyframes.map(k=>k.frame),[2,4]);
